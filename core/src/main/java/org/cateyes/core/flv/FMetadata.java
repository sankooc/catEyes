...
//			assert (type >>> 1) == 4;
	...
	boolean copyTag(FlvInputStream fis, DataOutputStream out, long pretime, List<Double> times, List<Double> pos) throws IOException {
		int type = fis.read();
		if (-1 == type) {
			return false;
		}
//		assert (type >>> 1) == 4;
	...
	public double copyTags(FlvInputStream fis, DataOutputStream out, double pretime, double presize) throws IOException {
